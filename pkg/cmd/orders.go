...
var scheduleOrderCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		timer := time.AfterFunc(targetTime.Add(-50*time.Millisecond).Sub(time.Now()), func() {
			for _, p := range pairs {
				go func(pair Pair) {
					o := types.SubmitOrder{
		...
		wg.Add(1)
		go func() {
			candidates := make(map[string]Pair)
			for _, p := range pairs {
				candidates[p.clientID] = p
			}
			for i := 0; i < 10000; i++ {
				time.Sleep(50 * time.Millisecond)

				timedCtx, cancel := context.WithTimeout(ctx, 10*time.Second)
				b, err := ex.QueryAccountBalances(timedCtx)
				if err != nil {
					cancel()
					continue
				}
				cancel()
				oxy := b["OXY"]
				available := oxy.Available.Float64()

				if i%40 == 0 {
					log.Infof("oxy total balance: %+v", oxy)
				}
				if available > 50 {
					log.Infof("oxy balance: %+v", oxy)
